...
pillow|Pillow is a "friendly fork" to the PIL. Development seems to have stalled on PIL, with last update made before several years, so it is been adopted as a replacement for PIL in several linux distributions. It has to be noted that Pillow and PIL cannot co-exist in the same environment and hence PIL has to be uninstalled before proceeding with Pillow.|Quick and easy to perform simple image augmentations|Native to Linux, only works on other OS's through workarounds
PIL|PIL (Python Imaging Library) supports opening, manipulating and saving the images in many file formats. It supports various image manipulations like filtering, enhancing, masking, handling transparency, additions and the like.||Lack of support, outdated<br/>Lack of advanced image analysis tools<br/>Needs additional libraries to perform video processing|
